...
var Debug = false // display incorrectly reformatted source and exit
...
func processFile(filename string, useStdin bool) error {
	...
	for _, Fix := range fixes {
		...
		if Fix.F(newFile) {
			...
			newFile, err = parser.ParseFile(FileSet, filename, newSrc, ParserMode)
			if err != nil {
				if Debug {
					fmt.Printf("%s", newSrc)
					report(err)
...
func gofmt(n interface{}) string {
	gofmtBuf.Reset()
	if err := format.Node(&gofmtBuf, FileSet, n); err != nil {
		return "<" + err.Error() + ">"
	}
